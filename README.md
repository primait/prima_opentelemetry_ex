...
You can disable the instrumentation for one or more external libraries by setting the `exclude` key like in the following example:

```elixir
config :prima_opentelemetry_ex, exclude: [:ecto, :telepoison, :teleplug, :absinthe]
```
